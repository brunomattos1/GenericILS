#=
    ...
    #     # Primeiro eu tento uma concatenaÃ§Ã£o, em que estendo $sol.routes[r][1:pos-1] -> $customer gerando um label forward
    ...
    #         # Se a inserÃ§Ã£o Ã© numa parte inviÃ¡vel da rota, nÃ£o hÃ¡ perda de viabilidade
    #         if pos > sol.lastFeasibleF[r]
    #             dFeasForward = 0
    #         else # Se a inserÃ§Ã£o Ã© numa parte viavel da rota, eu perco 1 arco viÃ¡vel que conecta $sol.routes[r][pos-1] ao $sol.routes[r][pos]
    #             dFeasForward = - 1
    #         end
    #         # if pos < sol.lastFeasibleF[r]
    #         #     dFeasForward = -1
    ...
    #         # A inserÃ§Ã£o Ã© numa regiÃ£o vÃ­avel? Se sim, perco um arco de viabilidade
    #         if pos > length(sol.routes[r]) + 1 - sol.lastFeasibleB[r]
    #             dFeasBackward = -1
    #         else # SenÃ£o, nÃ£o perco viabilidade
    #             dFeasBackward = 0
    #         end
    ...
    #     # nFwd: arcos viÃ¡veis sobreviventes antes da ponte
    #     nFwd = min(pos - 2, sol.lastFeasibleF[r] - 1)
    #     bridgeForward.cost < Inf && (nFwd += 1)
    #     # nBwd: arcos viÃ¡veis sobreviventes apÃ³s a ponte
    ...
    #     # nFwd: arcos antes da inserÃ§Ã£o (inalterados) + a^- + a^+
    #     nFwd = min(pos - 2, sol.lastFeasibleF[r] - 1)
    #     vecL.cost  < Inf && (nFwd += 1)
    #     vecL2.cost < Inf && (nFwd += 1)

    #     # nBwd: arcos apÃ³s a inserÃ§Ã£o (inalterados) + a^+ + a^-
    ...
    #     # nBwd: arcos apÃ³s v_{pos+1} (inalterados) + a^+ + a^-
    #     nBwd = min(lenR - 1 - pos, sol.lastFeasibleB[r] - 1)
    #     cevL.cost  < Inf && (nBwd += 1)
    #     cevL2.cost < Inf && (nBwd += 1)
    ...
    #     # Bridge arc: v_{pos-1} â†’ v_{pos+2}  (pula pos e pos+1)
    #     bridgeForward = myExtendAlongArc(solver.res.customResource,
    #         sol.forwardLabels[r][pos-1],
    ...
    #     # nBwd: arcos apÃ³s v_{pos+2} (inalterados) + ponte backward
    ...
    #     # nBwd: arcos apÃ³s v_{pos} (inalterados) + a3 + a2 + a1 (backward)
    #     nBwd = min(lenR - pos, sol.lastFeasibleB[r] - 1)
    ...
    #     # ExtensÃµes forward: ...routes[r][pos-1] -> c1 -> c2 -> routes[r][pos]
    ...
    #     # ExtensÃµes backward: routes[r][pos] -> c2 -> c1 -> routes[r][pos-1]
    ...
    #     # ConcatenaÃ§Ã£o direta: forward(atÃ© c2) + backward(de routes[r][pos])
    #     concat = myConcatenationCost(solver.res.customResource,
    ...
    #             insertionLabelF2.cost < Inf && (dFeasForward += 1)
    #             insertionLabelF3.cost < Inf && (dFeasForward += 1)
    #         else  # pos <= lff: arco routes[r][pos-1]->routes[r][pos] era viÃ¡vel
    #             dFeasForward = -1
    #             insertionLabelF1.cost < Inf && (dFeasForward += 1)
    #             insertionLabelF2.cost < Inf && (dFeasForward += 1)
    #             insertionLabelF3.cost < Inf && (dFeasForward += 1)
    ...
    #             insertionLabelB2.cost < Inf && (dFeasBackward += 1)
    #             insertionLabelB3.cost < Inf && (dFeasBackward += 1)
    #         else  # pos > bfbThreshold: arco routes[r][pos]->routes[r][pos-1] era viÃ¡vel backward
    #             dFeasBackward = -1
    #             insertionLabelB1.cost < Inf && (dFeasBackward += 1)
    #             insertionLabelB2.cost < Inf && (dFeasBackward += 1)
    #             insertionLabelB3.cost < Inf && (dFeasBackward += 1)
