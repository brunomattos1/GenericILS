# Identical in spirit to Algorithms/{NILS,ILS}/SetPartitioning.jl: collect
# every feasible route ever produced during education into a pool, then solve
# an exact Set Partitioning MIP over that pool at the end of the run.
# HGS's population naturally accumulates a more diverse pool than a single
# trajectory (many distinct individuals across generations, not one evolving
# solution), so it stands to benefit at least as much from this step.
