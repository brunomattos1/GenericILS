            ...
            for i = 2:length(sol.routes[r1]) - 1
                for j = 2:length(sol.routes[r2])
                    ...
                    if improvement
                        ...
                        bestCost = cost
                        bestInfeas = infeas
                        # applyMoveInterShift10!(solver, bestCost, bestViol, bestResViolR1, bestResViolR2, bestR1, bestR2, bestI, bestJ)
                        # return true
                    end
                end
            end
...
function interSwap11!2(solver::Solver)
    flag = false
    routesIdx = shuffle!(solver.seed, Int[i for i = 1:length(solver.currSol.routes)])
    for r1 in routesIdx#1:length(sol.routes)
        bestI = 0
        ...
        routes = getRoutes(sol)
        bestCost = getCost(sol)
        bestFeas = sol.feasiblesF[r1] + sol.feasiblesB[r1]
        for r2 in routesIdx
            if r1 == r2
                continue
            end
            bestFeas += sol.feasiblesF[r2] + sol.feasiblesB[r2]
            for i = 2:length(sol.routes[r1]) - 1
                for j = 2:length(sol.routes[r2]) - 1
                    cost, feas = evalInterSwap11(sol.cost, routes, solver, r1, r2, i, j)
                    improvement = improved(cost, bestCost, feas, bestFeas)
                    if improvement
                        bestI = i
                        bestJ = j
                        bestR1 = r1
