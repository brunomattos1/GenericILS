...
function extendAlongArc(res::CustomResource, label::ForwardLabel, a::Tuple{Int, Int})
    ...
    if k_ == 0
        ...
        return (ForwardState(res.nv,cost_,load_,dist_), 0.0) #colocar um return infeasible aqui: there is no vehicle with enough capacity for the path
    end
...
function extendAlongArc(res::CustomResource, label::BackwardLabel, a::Tuple{Int, Int})
    ...
    if k_ == 0
        ...
        return (BackwardState(res.nv,cost_,load_,dist_), 0.0) #colocar um return infeasible aqui: there is no vehicle with enough capacity for the path
    end
