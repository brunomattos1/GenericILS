...
function main(data::DataMDVRP,restarts::Int,outerIterMax::Int,innerIterMax::Int,seed::Int;
    ...
    dataHeuristic = ProblemData(customers, null_cost, K)
    
    # Standard Resource 
    d_ = Create_Demand_Matrix(data) #head demand of product 1
    stdRes = StandardResource(d_, Float64[0.0 for i = 1:length(customers)+1], Float64[Q for i = 1:length(customers)+1]) # 0.0 e n são os bounds nos vertices (grafo RCSPP)
